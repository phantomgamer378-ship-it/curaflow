      ...
      <div className="dashboard-panel" style={{ marginTop: "2rem" }}>
        {isLoading ? (
          ...
          <table style={{ width: "100%", borderCollapse: "collapse", textAlign: "left" }}>
            ...
            <tbody>
              {appointments.map((apt: any) => {
                ...
                if (apt.status === "in_consultation") { badgeColor = "var(--color-amber-100)"; badgeText = "var(--color-amber-700)"; }
                ...
                return (
                  <tr key={apt.id} style={{ borderBottom: "1px solid var(--color-slate-100)", transition: "background-color 0.2s" }} className="table-row-hover">
                    ...
                    <td style={{ padding: "1rem", fontWeight: 500 }}>{apt.patient?.profile?.name || "Unknown"}</td>
                    <td style={{ padding: "1rem" }}>{apt.patient?.profile?.phone || "—"}</td>
          ...
          <div style={{ padding: "4rem 1rem", textAlign: "center", color: "var(--color-slate-500)", display: "flex", flexDirection: "column", alignItems: "center", gap: "1rem" }}>
            ...
            <p style={{ margin: 0, fontSize: "0.875rem", color: "var(--color-slate-400)" }}>When patients book with you, they will appear here.</p>
          </div>
        )}
      </div>
    </main>
